...
router.post("/register", async (req, res) => {
  try {
    ...
    const { email, password, name, phone, batch, department } = req.body || {};
    ...
    await pool.query(
      `INSERT INTO profiles (id, name, phone, batch, department, verified, approved, blocked, profile_pending, social_links)
       VALUES (?, ?, ?, ?, ?, false, false, false, false, NULL)`,
      [userId, name, phone || null, batch || null, department || null]
    );
...
router.post("/login", async (req, res) => {
  try {
    ...
    const userOut = { ...profile, email: user.email, verified: true };
  ...
  try {
    ...
    Object.entries(allowed).forEach(([apiKey, dbKey]) => {
      if (Object.prototype.hasOwnProperty.call(req.body || {}, apiKey)) {
        updates.push(`\`${dbKey}\` = ?`);
        const value =
          apiKey === "socialLinks" && req.body[apiKey] ? JSON.stringify(req.body[apiKey]) : req.body[apiKey];
